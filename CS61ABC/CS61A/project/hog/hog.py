...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for i in range(1, 11):
        if i == 1:
            max_score = make_averaged(roll_dice, total_samples)(i, dice)
            max_num = i
        else:
...
def hefty_hogs_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    if hefty_hogs(score, opponent_score)  >= threshold or hog_pile(score, opponent_score) != 0:
        return 0
    return num_rolls
    # BEGIN PROBLEM 10
    # END PROBLEM 10
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    effect (i.e. if rolling 0 would result in a Hog Pile bonus).
    It also returns 0 dice if it gives at least THRESHOLD points.
    Otherwise, it returns NUM_ROLLS.
    """
    potential_score = hefty_hogs(score, opponent_score)
    # 模拟 roll 0 后玩家最终的得分为 score + hefty_hogs 获得的值，
    # 再加上可能的 Hog Pile bonus
    if hog_pile(score + potential_score, opponent_score) != 0 or potential_score >= threshold:
        return 0
    return num_rolls
