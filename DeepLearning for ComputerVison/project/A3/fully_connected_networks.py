...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        # 计算dx: dout @ W^T，然后恢复原始形状
        dx = dout.mm(w.t())  # N x M @ M x D -> N x D
        dx = dx.reshape(x.shape)  # 恢复原始形状
        
        # 计算dw: X^T @ dout
        dw = x_reshaped.t().mm(dout)  # D x N @ N x M -> D x M
        
        # 计算db: 对dout的列求和
        db = dout.sum(dim=0)  # N x M -> M
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        #######################################################################
        # Replace "pass" statement with your code
        # 在FullyConnectedNet类的__init__方法中
    ...
    def loss(self, X, y=None):
        ...
        # 1. 前向传播通过所有隐藏层
        # {linear - relu - [dropout]} x (L-1)
        for i in range(self.num_layers - 1):
            # Linear + ReLU
            out, cache = Linear_ReLU.forward(out, 
                                          self.params[f'W{i+1}'], 
                                          self.params[f'b{i+1}'])
            caches.append(cache)
        ...
        # 2. 最后一层只需要linear变换
        scores, cache = Linear.forward(out, 
        ...
        # 3.1 最后一层(只有linear)的反向传播
        dx, dw, db = Linear.backward(dx, caches[cache_idx])
        cache_idx -= 1
        ...
        for i in range(self.num_layers-2, -1, -1):
            # 如果使用了dropout，先处理dropout层
            if self.use_dropout:
                dx = Dropout.backward(dx, caches[cache_idx])
                cache_idx -= 1
            
            # Linear+ReLU的反向传播
            dx, dw, db = Linear_ReLU.backward(dx, caches[cache_idx])
            cache_idx -= 1
