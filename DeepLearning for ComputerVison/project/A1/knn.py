...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Compute the squared Euclidean distance using nested loops
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flat = x_train.view(num_train, -1)
    x_test_flat = x_test.view(num_test, -1)
    # Compute the squared Euclidean distance using two broadcast sums and a matrix multiply
    x_train_squared = torch.sum(x_train_flat ** 2, dim=1, keepdim=True)  # Shape: (num_train, 1)
    ...
    dists = x_train_squared - 2 * cross_term + x_test_squared.t()
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for j in range(num_test):
        ...
        # Find the label with the maximum count (in case of tie, the smallest label is chosen)
        max_count = torch.max(counts)
        max_labels = labels[counts == max_count]
        y_pred[j] = torch.min(max_labels)
...
def knn_cross_validate(
    ...
    for fold in range(num_folds):
        ...
        # 准备训练集，合并除当前折叠之外的所有数据
        x_train_cv = torch.cat(x_train_folds[:fold] + x_train_folds[fold+1:])
        y_train_cv = torch.cat(y_train_folds[:fold] + y_train_folds[fold+1:])

        # 对于每个 k 值，训练并评估模型
        for k in k_choices:
            classifier = KnnClassifier(x_train_cv, y_train_cv)
            y_pred = classifier.predict(x_valid, k=k)
            num_correct = (y_pred == y_valid).sum().item()
            accuracy = 100.0 * num_correct / y_valid.shape[0]
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    """
    best_k = 0
    best_accuracy = -1  # 初始化best_accuracy为一个很小的值
    ##########################################################################
