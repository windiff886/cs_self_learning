...
def svm_loss_vectorized(
    ...
    margins[margins > 0] = 1
    valid_margins_count = margins.sum(dim=1)
    margins[torch.arange(num_train), y] -= valid_margins_count
...
def train_linear_classifier(
    ...
    for it in range(num_iters):
        ...
        # perform parameter update
        W -= learning_rate * grad  # 使用梯度和学习率更新权重
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    scores = X.mm(W)  # 计算分数矩阵 (N, C)
    y_pred = torch.argmax(scores, dim=1)  # 找到每行最大值的索引
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-2,5e-1, 2e-1, 10] # [1e-2, 5e-1] 
    regularization_strengths = [1e-4, 1e-3, 10]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
