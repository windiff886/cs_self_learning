...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    # Compute the forward pass
    hidden = X.mm(W1) + b1  # First layer: FC
    hidden = hidden.clamp(min=0)  # ReLU activation
    scores = hidden.mm(W2) + b2  # Second layer: FC
...
def nn_forward_backward(
    ...
    # 计算 softmax loss
    scores_shifted = scores - scores.max(dim=1, keepdim=True)[0]  # 数值稳定性
